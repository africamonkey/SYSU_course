...
void welcome() {
	puts("");
	puts("AfricamonkeyOS v8.0.0 built-in shell");
...
void calc_disk(unsigned char id) {
	id = id - 1;
	zhumian = id / 18;
	citou = zhumian % 2;
	zhumian = zhumian / 2;
...
void read_disk(unsigned char id, unsigned char size, int memSeg, int memAddr) {
	calc_disk(id);
...
void ls() {
	char tmp;
	int size, i, j, k, addr, ces;
	asm mov ax, es
	asm mov ces, ax
	read_disk(2, 1, ces, 0x8000); /* sector 2: user program */
